...
public class ProcessUtil {
    ...
    public static void killTaskByName1(String taskNameParm){
        /*
         * 下面先是获取进程列表
         */
...
//                System.out.println(taskName[i]+" "+taskPid[i]+" "+taskSessionName[i]+" "+taskSession[i]+" "+taskNec[i]);
                //根据进程名称找到PID，杀掉进程
                if(taskName[i].contains(taskNameParm+".exe")) {
//                if(taskName[i].contains("notepad.exe")) {
//                if(taskName[i].contains("ffmpeg.exe")) {
                    //杀死进程
                    Process p = Runtime.getRuntime().exec("taskkill /F /PID "+taskPid[i]);
                    int result = p.waitFor();
                    if(result==0){
                        System.out.println("杀死"+taskNameParm+"进程，进程Pid为："+taskPid[i]);
                    }
                }
            }
        } catch (Exception e) {
    ...
    public static void closeAllFFmpeg() throws Exception {
        String taskName = "ffmpeg";
        killTaskByName1(taskName);
    }
